...
export default function RoomCard({ room, currentUserId }: RoomCardProps) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-md transition-all duration-200 hover:border-indigo-300 cursor-pointer group" >
        {/* Header */ }
...
{
    isHost && (
        <Crown className="h-4 w-4 text-yellow-500" title = "You're the host" />
              )
}
</div>
    < div className = "flex items-center space-x-4 text-sm text-gray-500" >
        <span className="flex items-center space-x-1" >
            <Users className="h-4 w-4" />
                ...
                < span > { getAIModelDisplay(room.ai_model) } </span>
                < span > { getPaymentModelDisplay(room.payment_model) } </span>
                </div>
                </div>
